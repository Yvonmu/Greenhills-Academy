...
import FormDataModel from '@/Models/MissionModel';
...
async function getHandler(req: NextApiRequest, res: NextApiResponse<any>) {
    try {
        const formDataList = await FormDataModel.find({}).sort({createdAt:-1});
...
async function postHandler(req: NextApiRequest, res: NextApiResponse<any>) {
    try {
        ...
        if (Array.isArray(body)) {
            formData = await FormDataModel.insertMany(body);
        } else {
            formData = await FormDataModel.create(body);
        }
