...
async function getHandler(req: NextApiRequest, res: NextApiResponse<any>) {
  try {
    const queries = await SubscriptionModel.find({}).sort({ createdAt: -1 });
...
async function postHandler(req: NextApiRequest, res: NextApiResponse<any>) {
  try {
    ...
    const newQuery = new SubscriptionModel({ name, email, message });
    await newQuery.save();
...
async function deleteHandler(req: NextApiRequest, res: NextApiResponse<any>) {
  try {
    ...
    const deletedQuery = await SubscriptionModel.findByIdAndDelete(id);
    if (!deletedQuery) {
...
async function putHandler(req: NextApiRequest, res: NextApiResponse<any>) {
  try {
    ...
    const updatedQuery = await SubscriptionModel.findByIdAndUpdate(id, { replied }, { new: true });
